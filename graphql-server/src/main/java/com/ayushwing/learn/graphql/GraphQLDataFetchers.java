package com.ayushwing.learn.graphql;

import java.util.Arrays;
import java.util.List;
import java.util.Map;

import org.springframework.stereotype.Component;

import com.google.common.collect.ImmutableMap;

import graphql.schema.DataFetcher;

@Component
public class GraphQLDataFetchers {

	private static List<Map<String, Object>> books = Arrays.asList(
			ImmutableMap.of("id", 1, "name", "Harry Potter and the Philosopher's Stone", "authorId", 1),
			ImmutableMap.of("id", 2, "name", "Moby Dick", "authorId", 2),
			ImmutableMap.of("id", 3, "name", "Interview with the vampire", "pageCount", "371", "authorId", 3));

	private static List<Map<String, Object>> authors = Arrays.asList(
			ImmutableMap.of("id", 1, "firstName", "Joanne", "lastName", "Rowling"),
			ImmutableMap.of("id", 2, "firstName", "Herman", "lastName", "Melville"),
			ImmutableMap.of("id", 3, "firstName", "Anne", "lastName", "Rice"));

	public DataFetcher<?> getBookByIdDataFetcher() {
		return dataFetchingEnvironment -> {
			Integer bookId = Integer.parseInt(dataFetchingEnvironment.getArgument("id"));
			return books.stream().filter(book -> book.get("id").equals(bookId)).findFirst().orElse(null);
		};
	}

	public DataFetcher<?> getAutherDataFetcher() {
		return dataFetchingEnvironment -> {
			Map<String, Object> book = dataFetchingEnvironment.getSource();
			Integer authorId = (Integer) book.get("authorId");
			return authors.stream().filter(author -> author.get("id").equals(authorId)).findFirst().orElse(null);
		};
	}

}
